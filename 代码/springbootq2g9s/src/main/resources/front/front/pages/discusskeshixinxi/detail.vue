...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusskeshixinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusskeshixinxiCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusskeshixinxi');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discusskeshixinxi');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discusskeshixinxi', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusskeshixinxi', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscusskeshixinxi/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discusskeshixinxi', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discusskeshixinxi', this.detail);
				this.$utils.msg('审核成功');
